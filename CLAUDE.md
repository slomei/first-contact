...
| File | Description |
|------|-------------|
| `.env` / `.env.example` | API keys: `ANTHROPIC_API_KEY` (required), `DISCORD_BOT_TOKEN`, `DISCORD_USER_ID`, `TELEGRAM_BOT_TOKEN`, `TELEGRAM_USER_ID`, `OPENAI_API_KEY`, `GEMINI_API_KEY` (all optional). |
| `config.json` / `config.example.json` | Generated by onboarding. Provider selection (`"provider"`), model tier overrides (`"model_tiers"`), briefing schedule, email notification prefs, job scan queries, user profile, daemon settings (including `auto_start_web_ui`, `auto_start_discord`, `auto_start_telegram`), rate limits, custom prompt. |
