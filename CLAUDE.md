...
| `config.json` / `config.example.json` | Generated by onboarding. Briefing schedule, email notification prefs, job scan queries, user profile, daemon settings (including `auto_start_web_ui`, `auto_start_discord`, `auto_start_telegram`), rate limits, custom prompt. |
