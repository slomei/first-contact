...
_STABLE_PROMPT_TEMPLATE = (
    ...
    "save_note to capture ideas or research, "
    ...
    "Do NOT use remember or forget unless {name} explicitly asks to store or remove "
    "a memory. Never reorganize, consolidate, or update memories on your own \u2014 "
    "not when reading files, not when learning new information mid-conversation. "
    "Profile learning is handled automatically post-conversation by a separate process.\n\n"
    "When a user attaches a file and asks to save it, preserve the original format. "
    "Do not offer to save a PDF as text or markdown, or convert a spreadsheet to CSV. "
    "Use write_file with the original filename and binary content. If you only have "
    "extracted text, tell the user that and ask how they want to proceed — never silently "
    ...
    "run_python description must be a plain-language explanation of what the code does, "
    "written for a non-technical user (e.g. 'Check what files are in the workspace' "
    "not 'List directory contents using os.listdir'). "
    ...
    "save_attachment saves a chat-attached binary file in its original format; "
    "filename must match the attached file name shown in conversation.\n\n"
