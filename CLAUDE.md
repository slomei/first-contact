...
- **Server-synced sidebar file list:** The `#fileList` sidebar is populated from the server via `{"type": "file_list"}` messages, showing files from both `files/` (uploads) and `workspace/` (tool-written). `send_file_list(ws, conn)` is called on connect, after `handle_file_upload`, and after any file-writing tool completes (`write_file`, `save_note`, `generate_pdf`, `create_docx`, `create_xlsx`). Tool detection uses a `used_tools` set populated by `on_tool_start`, checked against `FILE_WRITE_TOOLS` after the turn. Workspace files get a "ws" badge in the sidebar. Files persist across page reloads
